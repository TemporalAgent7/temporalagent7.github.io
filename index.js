...
// ======== Localization
let loc = JSON.parse(fs.readFileSync(__dirname + '/zhatvash2/localization.json'));
...
// ======== Skills
let skills = JSON.parse(fs.readFileSync(__dirname + '/zhatvash2/GSSkill.json'));
...
// ======== Rank Modifiers
let rankModifiers = JSON.parse(fs.readFileSync(__dirname + '/zhatvash2/GSRank.json'));
...
// ======== Level Modifiers
let levelModifiers = JSON.parse(fs.readFileSync(__dirname + '/zhatvash2/GSLevel.json'));
...
// ======== Characters
let characters = JSON.parse(fs.readFileSync(__dirname + '/zhatvash2/GSCharacter.json'));
...
for (let cr in characters) {
	if (characters[cr].Type != 'Disabled') {
		let crew = {
			...
			description: L(characters[cr].Description),
        ...
        fs.copyFileSync(`${__dirname}/zhatvash2/assets/${crew.icon}.png`, `assets/${crew.icon}.png`)
